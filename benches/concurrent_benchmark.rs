...
fn setup_log(level: LogLevel) {
    LOG_INIT.call_once(|| {
        let _ = env_logger::Builder::new()
            ...
            .filter_module("concurrent_benchmark", match level {
...
fn setup_loguru(level: LoguruLogLevel) -> Logger {
    // Configure loguru for thread-safety and async processing if available
    Logger::new()
...
fn bench_log_concurrent(level: LogLevel, message: &str, thread_count: usize, logs_per_thread: usize) {
...
fn bench_slog_concurrent(logger: &SlogLogger, level: SlogLevel, message: &str, thread_count: usize, logs_per_thread: usize) {
...
fn bench_tracing_concurrent(level: TracingLevel, message: &str, thread_count: usize, logs_per_thread: usize) {
    let barrier = Arc::new(Barrier::new(thread_count + 1));
    let mut handles = Vec::with_capacity(thread_count);
...
fn bench_loguru_concurrent(logger: &Logger, level: LoguruLogLevel, message: &str, thread_count: usize, logs_per_thread: usize) {
    let logger = Arc::new(logger.clone());
    let barrier = Arc::new(Barrier::new(thread_count + 1));
    let mut handles = Vec::with_capacity(thread_count);
    ...
    for thread_id in 0..thread_count {
        ...
        let handle = thread::spawn(move || {
            ...
            for i in 0..logs_per_thread {
                let log_message = format!("{} - Log {}", thread_message, i);
                match level {
                    LoguruLogLevel::CRITICAL => thread_logger.critical(&log_message),
                    LoguruLogLevel::ERROR => thread_logger.error(&log_message),
                    LoguruLogLevel::WARNING => thread_logger.warning(&log_message),
                    LoguruLogLevel::SUCCESS => thread_logger.success(&log_message),
                    LoguruLogLevel::INFO => thread_logger.info(&log_message),
                    LoguruLogLevel::DEBUG => thread_logger.debug(&log_message),
                    LoguruLogLevel::TRACE => thread_logger.trace(&log_message),
                }
            }
        });
    ...
    for &thread_count in thread_counts.iter() {
        for (level_name, log_level, slog_level, tracing_level, loguru_level) in log_levels.iter() {
            ...
            group.bench_function(
                ...
                |b| {
                    b.iter(|| bench_log_concurrent(*log_level, &message, thread_count, logs_per_thread));
                },
            );
            ...
            group.bench_function(
                ...
                |b| {
                    b.iter(|| bench_slog_concurrent(&slog_logger, *slog_level, &message, thread_count, logs_per_thread));
                },
            );
            ...
            group.bench_function(
                ...
                |b| {
                    b.iter(|| bench_tracing_concurrent(*tracing_level, &message, thread_count, logs_per_thread));
                },
            );
            ...
            group.bench_function(
                ...
                |b| {
                    b.iter(|| bench_loguru_concurrent(&loguru_logger, *loguru_level, &message, thread_count, logs_per_thread));
                },
            );
        }
    }
