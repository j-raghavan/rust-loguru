...
fn setup_log4rs() -> io::Result<()> {
    ...
    // Initialize the logger with the config, but skip if already set
    match log4rs::init_config(config) {
        Ok(_) => Ok(()),
        Err(e) => {
            if e.to_string().contains("SetLoggerError") {
                Ok(())
            } else {
...
fn setup_tracing_with_rotation() -> io::Result<(
    ...
    // Return the non-blocking writer and guard instead of initializing a global subscriber
...
fn setup_loguru_with_rotation() -> io::Result<Logger> {
    ...
    let handler = FileHandler::new(log_file.to_str().unwrap())?.with_rotation(LOG_FILE_SIZE, 5);
    let mut logger = Logger::new(LoguruLogLevel::Info);
    logger.add_handler(new_handler_ref(handler));
    Ok(logger)
}
...
fn bench_tracing_with_rotation(
    writer: &tracing_appender::non_blocking::NonBlocking,
    message: &str,
) -> io::Result<()> {
    use tracing_subscriber::fmt;

    // Create a local subscriber that uses our non-blocking writer
    ...
    // Use a local tracing scope instead of a global one
...
fn benchmark_file_rotation(c: &mut Criterion) {
    ...
    // tracing benchmark - fixed version
    {
        group.bench_function(BenchmarkId::new("tracing", "file_rotation"), |b| {
            b.iter_custom(|iters| {
                ...
                for _ in 0..iters {
                    let _ = setup_log_dir();
                    let (writer, _guard) = setup_tracing_with_rotation().unwrap();
                    let start = std::time::Instant::now();
                    let _ = bench_tracing_with_rotation(&writer, &message);
                    total_duration += start.elapsed();
                }
                total_duration
            });
